import{r as E,c as v,i as w}from"./sprinkler-utils.js";class D extends HTMLElement{constructor(){super(),this._connected=!1,this._showDisabled=!1,this._pendingRequestId=null,this._activeDialog=null,this._zonePrefix=null,this._isDev=!1}_getZonePrefix(){return this.config?.entity?this.config.entity.replace(/_\d+$/,""):null}_getZoneEntity(t){const e=this._getZonePrefix();return e?`${e}_${String(t).padStart(2,"0")}`:null}setConfig(t){this.config=t}set hass(t){this._hass=t,requestAnimationFrame(()=>{const e=this.querySelector("ha-card"),i=e.getBoundingClientRect().width;e.style.maxWidth=`${i}px`}),this._initialized||(this.render(),this._initialized=!0),!this._feedbackRegistered&&t&&E(t),this._zonePrefix||(this._zonePrefix=this._getZonePrefix()),this.update()}connectedCallback(){this._connected||(requestAnimationFrame(()=>{const t=this.querySelector("ha-card"),e=t.getBoundingClientRect().width;t.style.maxWidth=`${e}px`}),this._connected=!0,this._hass.connection.subscribeEvents(t=>{const e=t.data;console.log("Event data:",e),console.log("Pending:",this._pendingRequestId),e&&e.request_id===this._pendingRequestId&&(e.user_id&&e.user_id!==this._hass.user?.id||(this._pendingRequestId=null,this._requestTimeout&&(clearTimeout(this._requestTimeout),this._requestTimeout=null)))},"sprinkler_ui_feedback").then(t=>{this._unsubscribe=t}),console.log("HA version:",this._hass.config.version))}_closeActiveDialog(){this._activeDialog&&(this._activeDialog.close(),this._activeDialog=null)}getCardSize(){return 6}openEditDialog(t=null){const e=document.createElement("ha-dialog"),i=Number(this._hass.states["input_number.sprinkler_capacity"]?.state)||1;document.body.appendChild(e),setTimeout(()=>e.show(),0);const n=t===null;let a="",s=1,d=1,l=10,u=30,p=!0,r="";if(!n){const o=this._getZoneEntity(t),_=this._hass.states[o];if(!_)return;const x=_.attributes;a=x.zone_name||"",s=x.load||1,d=x.zone_factor||1,l=x.precipitation_rate_mm_per_hour||10,u=Math.floor((x.default_duration||600)/60),p=x.enabled??!0,r=x.switch||""}const c=Object.values(this._hass.states).filter(o=>o.entity_id.startsWith("switch.")).sort((o,_)=>o.entity_id.localeCompare(_.entity_id)).map(o=>`
    ...
    `).join("");e.innerHTML=`
      ...
      <div class="dialog-content">
        ...
        <div class="dialog-body">
          ...
          <div class="field">
            <label>
              <input id="enabled" type="checkbox" ${p?"checked":""}/>
              Aktiviert
            </label>
          </div>
    ...
    `;const g=e.querySelector("#duration"),b=e.querySelector("#factor"),f=e.querySelector("#precip"),y=e.querySelector("#load"),z=e.querySelector("#durationVal"),m=e.querySelector("#factorVal"),q=e.querySelector("#precipVal"),$=e.querySelector("#loadVal");g.addEventListener("input",o=>z.textContent=o.target.value),b.addEventListener("input",o=>m.textContent=Number(o.target.value).toFixed(1)),f.addEventListener("input",o=>q.textContent=o.target.value),y.addEventListener("input",o=>$.textContent=o.target.value),e.querySelector("#cancelBtn").addEventListener("click",()=>{e.close()}),e.querySelector("#saveBtn").addEventListener("click",()=>{this._activeDialog=e;const o={zone_id:t,name:e.querySelector("#zone_name").value,switch:e.querySelector("#switch").value,default_duration:Number(g.value)*60,load:Number(y.value),zone_factor:Number(b.value),precipitation_rate_mm_per_hour:Number(f.value),enabled:e.querySelector("#enabled").checked};v(this,n?"sprinkler_ui_zone_add":"sprinkler_ui_zone_update",{zone:o}),e.close()}),e.addEventListener("closed",()=>e.remove())}openDurationDialog(t,e,i){const n=document.createElement("ha-dialog");document.body.appendChild(n);const a=this._getZoneEntity(t);if(!a)return;const s=this._hass.states[a];if(!s)return;const d=s.attributes?.default_duration||600,l=`${a}_remaining`,u=this._hass.states[l],p=Number(u?.state)||d;let r=Math.max(1,Math.floor(e==="running"?p/60:d/60));const h=e==="running",c=e==="queued",g=h?"Restlaufzeit einstellen":"Laufzeit einstellen",b=h?"Setzen":c?"Übernehmen":"Start";n.innerHTML=`
      <style>
        ha-dialog {
    ...
    `,setTimeout(()=>{n.show()},0);const f=n.querySelector("#slider"),y=n.querySelector("#value");function z(){y.textContent=`${r} min`,f.value=r}f.addEventListener("change",m=>{r=Number(m.target.value),z()}),n.querySelector("#cancelBtn").addEventListener("click",()=>{n.close()}),h?(n.querySelector("#stopBtn").addEventListener("click",()=>{this._activeDialog=n,v(this,"sprinkler_ui_cancel_zone",{qe_id:i})}),n.querySelector("#extendBtn").addEventListener("click",()=>{this._activeDialog=n,v(this,"sprinkler_ui_extend_zone",{qe_id:i,duration:r*60})})):n.querySelector("#startBtn").addEventListener("click",()=>{const m=c?"sprinkler_ui_extend_zone":"sprinkler_ui_start_zone";this._activeDialog=n,v(this,m,{qe_id:i,duration:r*60})}),n.addEventListener("closed",()=>n.remove())}openDeleteDialog(t){const e=document.createElement("ha-dialog");document.body.appendChild(e),setTimeout(()=>e.show(),0),e.innerHTML=`
      <style>
    ...
    `,setTimeout(()=>e.show(),0),e.querySelector("#cancelBtn").addEventListener("click",()=>{e.close()}),e.querySelector("#confirmBtn").addEventListener("click",()=>{this._activeDialog=e,v(this,"sprinkler_ui_zone_delete",{zone_id:t})}),e.addEventListener("closed",()=>e.remove())}openCancelDialog(t,e){const i=document.createElement("ha-dialog");document.body.appendChild(i),setTimeout(()=>i.show(),0),i.innerHTML=`
      <style>
    ...
    `,setTimeout(()=>i.show(),0),i.querySelector("#cancelBtn").addEventListener("click",()=>{i.close()}),i.querySelector("#confirmBtn").addEventListener("click",()=>{this._activeDialog=i,v(this,"sprinkler_ui_cancel_zone",{qe_id:e})}),i.addEventListener("closed",()=>i.remove())}render(){const t=w(this._hass),e=this.config?.title??this._hass?.states[this.config?.entity]?.attributes?.title??"Sprinkler Zones",i=this._isDev?`${e} (Dev)`:e;this.innerHTML=`
      <ha-card>
        ${this.styles()}
