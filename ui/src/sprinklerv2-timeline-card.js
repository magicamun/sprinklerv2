...
*/
  ...
  set hass(hass) {
    console.log("SET HASS TRIGGERED");

    this._lastHass = hass;

    if (!this._connected) return;   // <---- DAS HINZUFÜGEN
  ...
  _render() {
    ...
    if (hash === this._lastEventsHash) {
      this.updateNowLine();   // Nur Now-Line bewegen
      return;
    }
